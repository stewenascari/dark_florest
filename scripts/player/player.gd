...
func horizontal_movement_env() -> void:
	var input_direction: float = Input.get_action_raw_strength("ui_right") - Input.get_action_raw_strength("ui_left")
	if !can_track_input or attacking:
		velocity.x = 0
		return
	velocity.x = input_direction * speed
	
func vertical_movement_evn() -> void:
...
func attack() -> void:
	var attck_condition = not attacking and not crouching and not defending
	if Input.is_action_just_pressed('attack') and attck_condition and is_on_floor():
		attacking = true
		player_sprite.attack = true
